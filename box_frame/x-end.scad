...
module x_end_motor(){
    ...
    mirror([0, 1, 0]) {
        x_end_base(
            [3, 3, min((bushing_xy[0] - 3) * 2, 3), 2], 
            len=29 + nut_trap_width, 
            offset=-5, 
            thru=false);
...
module x_end_base(vfillet=[3, 3, 3, 3], thru=true, len=0, offset=0){
    difference(){
        union(){
            translate([-10 - bushing_xy[0], -10 + len / 2 + offset, 30]) {
                cube_fillet(
                    [x_box_width, len, x_box_height], 
                    center=true, 
                    vertical=vfillet, 
                    top=[5, 3, 5, 3]);
            }
            ...
            translate([0, 0, 4 - bushing_xy[0]]) {
                ...
                // Nut trap
                translate([z_threaded_rod_x_offset, z_threaded_rod_y_offset, 5]) {
                    cube_fillet(
                        [nut_trap_length, nut_trap_width, 10], 
                        center = true, 
                        vertical=[8, 0, 0, 5]);
                }
                //}
        ...
        // belt hole
        translate([-14 - xy_delta / 2, -10 + len/2 + offset, 30]) {
            cube_fillet(
                [max(idler_width + 2, 11), len+5, 27], 
                center = true, 
                vertical=0, 
                top=[0, 1, 0, 1], 
                bottom=[0, 1, 0, 1], 
        ...
        translate([-10 - bushing_xy[0], offset, 0]) {
            if(thru == true){
                translate([0, -11, 6]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.2, 55);
                translate([0, -11, xaxis_rod_distance+6]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.2, 55);
            } else {
                translate([0, -7, 6]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.2, 55);
                translate([0, -7, xaxis_rod_distance+6]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.2, 55);
            }
        }
        translate([0, 0, 5 - bushing_xy[0]]) {  // m5 nut insert
            translate([z_threaded_rod_x_offset, z_threaded_rod_y_offset, 0]) rotate([0, 0, 10]){
                //rod
                translate([0, 0, -1]) cylinder(h=(4.1 / 2 + 5), r=z_threaded_rod_radius, $fn=32);
                //nut
                translate([0, 0, 9]) cylinder(r=z_threaded_rod_nut_radius, h=14.1, center = true, $fn=6);
...
module x_end_idler(){
    ...
    difference() {
        x_end_base(len=z_delta / 3 + 33 + nut_trap_width, offset=-10 - z_delta / 3);
        ...
        translate([-6 - x_box_width, 11, x_box_height/2 - idler_height/2]) {
            #cube([x_box_width + 1, nut_trap_width-2, idler_height]);
        }
    }
...
translate([0, -80, 0]) x_end_idler(thru=true);
translate([-50, 0, 0]) translate([-50, 0, 0]) x_end_motor();
